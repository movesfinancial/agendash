...
const notifyOnFailure = (agenda, notificationList, agendashEnv) => {
  ...
  if (!recipients.length || recipients.length < 1) {
    console.log('Failed to parse notification recipient list!');
    return;
  }
  ...
  const notify = () => {
    void (async () => {
      console.log(`Scanning agenda-managed jobs to notify admins about failed jobs ...`);
      ...
      try {
        ...
        if (!mongoDb) {
          console.log(`Cannot perform failed-job notifications: agenda._mdb not found!`);
          return;
        }
        ...
        while(await failedJobs.hasNext()) {
          ...
          if (!didSendNotification) {
            ...
            const subject = `${env ? `[${env}]` : ''}[Agenda] Job Failed: ${name}`
            const text = `${subject} @ ${failedAt.toISOString()}\n\nPlease see Agendash & Datadog for details. Job snapshot:\n${JSON.stringify(job, undefined, 2)} `
